...
def inference(loader, model):
    ...
    with torch.no_grad():
        ...
        for sample in pbar:
            ...
            softmax_predictions = torch.softmax(
                outputs, dim=-1).cpu().numpy().tolist()
    ...
    confusion_matrix_ = np.array_str(
        confusion_matrix(ground_truths, pred_classes, labels=[0, 1, 2, 3, 4, 5, 6, 7, 8]))  # class
    print(f'val: \n{confusion_matrix_}')

    cf_matrix = confusion_matrix(
        ground_truths, pred_classes)

...
args = EasyDict({
    ...
    'start_epoch': 1,
    'num_epochs': 25,
    'layers_list': ['layer3', 'layer4', 'fc'],  # r2plus1d_18
    # 'layers_list': ['fc6', 'fc7'],  # C3D
    ...
    'augmented_annotation_path': "../datasets/augmented_annotation_dict.json",
    'model_path': "../model_checkpoints/r2plus1d_augmented_9class/",
    'history_path': "../histories/history_r2plus1d_augmented_9class.txt"
})


def show_confusion_matrix(confusion_matrix, class_names, phase, epoch):
    ...
    plt.savefig("../confusion_matrix/r2plus1d_augmented_9class/" +
                str(phase) + "_confusion_matrix_acc_" + str(epoch) + ".png")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for sample in pbar:
                ...
                pbar.set_description('Phase: {} || Epoch: {} || Loss {:.5f} '.format(
                    phase, epoch, running_loss / train_n_total))
                train_n_total += 1
            ...
            print(phase, ' training complete in {:.0f}m {:.0f}s'.format(
            ...
            if phase == 'val':
                ...
                val_confusion_matrix = np.array_str(confusion_matrix(
                    val_ground_truths, val_pred_classes, labels=[0, 1, 2, 3, 4, 5, 6, 7, 8]))
                ...
                cf_matrix = confusion_matrix(
                    val_ground_truths, val_pred_classes)
                # 計算每個class的accuracy
                # https://stackoverflow.com/a/53824126/13369757
                ...
                plt.savefig("../confusion_matrix/r2plus1d_augmented_9class/" +
                            "val_confusion_matrix_" + str(epoch) + ".png")

                # show_confusion_matrix(df_cm, class_names, phase, epoch)
            ...
            if phase == 'train':
                ...
                train_confusion_matrix = np.array_str(confusion_matrix(
                    train_ground_truths, train_pred_classes, labels=[0, 1, 2, 3, 4, 5, 6, 7, 8]))
                ...
                # https://christianbernecker.medium.com/how-to-create-a-confusion-matrix-in-pytorch-38d06a7f04b7
                cf_matrix = confusion_matrix(
                    train_ground_truths, train_pred_classes)
                # 計算每個class的accuracy
                # https://stackoverflow.com/a/53824126/13369757
                per_cls_acc = cf_matrix.diagonal()/cf_matrix.sum(axis=1)
                class_names = [0, 1, 2, 3, 4, 5, 6, 7, 8]
                ...
                plt.ylabel("label (ground truth)")
                plt.savefig("../confusion_matrix/r2plus1d_augmented_9class/" +
                            "train_confusion_matrix_" + str(epoch) + ".png")
...
if __name__ == "__main__":
    ...
    # ANCHOR: import C3D Model
    # model = C3D(num_classes=args.num_classes, pretrained=args.pretrained)
    ...
    # ANCHOR: let C3D model parameters in fc6, fc7 to learn
    # let pretrained net's layer3, layer4, fc parameters to learn
    ...
    num_ftrs = model.fc.in_features  # return 512
    ...
    def sometimes(aug): return vidaug.Sometimes(0.5, aug)
    ...
    val_loader = DataLoader(
        dataset=val_subset, shuffle=False, batch_size=args.batch_size)
    ...
    print("Best Validation Loss: ", min(val_loss_history),
          "Epoch: ", val_loss_history.index(min(val_loss_history)))
    print("Best Training Loss: ", min(train_loss_history), "Epoch: ",
          train_loss_history.index(min(train_loss_history)))

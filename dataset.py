...
class BasketballDatasetTensor(Dataset):
    ...
    def __init__(self, annotation_dict, video_dir="../datasets/mp4/", data_dir="tensor-dataset/", transform=None, poseData=False):
        with open(annotation_dict) as f:
            self.video_list = list(json.load(f).items())
    ...
    def __getitem__(self, idx):
        ...
        if self.poseData:
            ...
            sample = {'video_id': video_id, 'video': video, 'action': torch.from_numpy(
                np.array(encoding[self.video_list[idx][1]])), 'class': self.video_list[idx][1]}
...
class BasketballDatasetNumpy(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.poseData:
            ...
            sample = {'video_id': video_id, 'video': torch.from_numpy(video), 'action': torch.from_numpy(
                np.array(encoding[self.video_list[idx][1]])), 'class': self.video_list[idx][1]}
        ...
        if ret:
            ...
            if padding_mode == 'zero':
                ...
                frames[:, index:, :, :] = frames[:, index -
                                                 1, :, :].view(channels, 1, height, width)
            break
...
def VideoToNumpy(video_id, data_dir="dataset/examples/", output_dir="numpy-dataset/"):
    ...
    video.release()
    assert len(video_frames) == 16
    np.save(output_dir+video_id+".npy",
            np.transpose(np.asarray(video_frames), (1, 0, 2, 3)))
...
def convertAllVideoTensor(path="dataset/annotation_dict.json", data_dir="dataset/examples/", output_dir="tensor-dataset/"):
    ...
    for video_id in video_list:
        ...
        print("Video: ", i)
        print(i/37085)
        VideoToTensor(video_id[0], data_dir, output_dir,
                      max_len=16, fps=10, padding_mode='last')
        i += 1
...
if __name__ == "__main__":
    ...
    basketball_dataset = BasketballDataset(annotation_dict="../datasets/annotation_dict.json",
                                           augmented_dict="../datasets/augmented_annotation_dict.json")
